...
jest.mock('@/contexts/GamePlayEasyContext', () => ({
  useGamePlayEasy: () => ({ changePhaseState: mockChangePhaseState }),
}))

const studentLevel1Phase5 = { name: 'Maria', level: 1, phase: 5 }
const studentLevel2 = { name: 'Ana', level: 2, phase: 3 }

describe('ThemeSelection (nivel1/[theme]/page)', () => {
  beforeEach(() => {
    localStorage.clear()
  ...
  it('renders logo, avatar and back button', () => {
    localStorage.setItem('aluno', JSON.stringify(studentLevel1Phase5))
  ...
  it('unlocks all phases when student is on a higher level', () => {
    localStorage.setItem('aluno', JSON.stringify(studentLevel2))
    render(<ThemeSelection params={{ theme: 'alimentos' }} />)
