...
import Nivel5 from "../page";
import { phases } from "../../../../nivel5/phases";
import {
...
const defaultContext = createGamePlayContext({
  currentVowel: { key: 1, letter: "MA", sound: "/audios/ma.mp3" },
  targetLetters: ["M", "A"],
});
...
describe("Nivel5 Jogo Page", () => {
  ...
  it("calls notFound for invalid theme", () => {
    expect(() => render(<Nivel5 params={{ theme: "invalid" }} />)).toThrow("NEXT_NOT_FOUND");
    expect(notFound).toHaveBeenCalled();
  });

  it("renders play button when not started", () => {
    render(<Nivel5 params={{ theme: "alimentos" }} />);
    expect(screen.getByAltText("play")).toBeInTheDocument();
    expect(screen.queryByText(/Nível 5 - Fase/)).not.toBeInTheDocument();
  });
  ...
  it("renders layout elements", () => {
    ...
    expect(backButton).toHaveAttribute("href", "nivel5/alimentos");
  });
  ...
  it("renders game UI in early phase (syllable buttons)", () => {
    ...
    render(<Nivel5 params={{ theme: "alimentos" }} />);
    expect(screen.getByText("Nível 5 - Fase 1")).toBeInTheDocument();
  ...
  it("renders game UI in late phase (letter grid)", () => {
    ...
    render(<Nivel5 params={{ theme: "alimentos" }} />);
    expect(screen.getByText("Nível 5 - Fase 3")).toBeInTheDocument();
  ...
  it("renders phrase buttons and calls handleClickWord in phrase phase", () => {
    ...
    render(<Nivel5 params={{ theme: "alimentos" }} />);
    const phraseButton = screen.getByText("LILO");
