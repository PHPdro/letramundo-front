    ...
    <div className="h-screen bg-[#b6d5f0]">
      <div className="flex flex-col h-screen p-6">
        <div className="flex justify-between z-10">
          <BackButton url="niveis/praia" color="blue" />
          <img src="/logo-transparente.png" alt="Logo" className=" z-10 w-[67px] h-[50px]" />
          <Avatar />
        </div>
        <h1 className="text-3xl font-medium text-center text-white mb-10 z-10">Nível 11</h1>
        <div className="grid grid-cols-4 gap-8 z-10 items-center max-w-[400px] mx-auto">
          {Array.from({ length: 8 }, (_, index) => (
            <Link href={`praia/jogo`} key={index}>
              <button
                onClick={() => changePhaseState(index + 1)}
                disabled={student?.phase < index + 1 && student.level === 11}
                className="w-[70px] h-[65px] p-2 rounded-full text-center items-center flex justify-center font-mono"
                style={{
                  backgroundColor:
                    student?.phase - 1 < index + 1 && student.level === 11 ? "#f8fafc" : "#80cd3b",
                }}
              >
                {student?.phase < index + 1 && student.level === 11 ? <LockOutlined /> : index + 1}
              </button>
            </Link>
          ))}
        </div>
      ...
      <img
        src="/praia-bg-niveis.png"
        alt="Praia de tema de fundo"
