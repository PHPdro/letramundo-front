...
import Nivel11 from "../page";
import { phases } from "../../../../nivel11/phases";
import {
...
const defaultContext = createGamePlayContext({
  currentVowel: { key: 1, letter: "DA", sound: "/audios/da.mp3" },
  targetLetters: ["D", "A"],
});
...
describe("Nivel11 Jogo Page", () => {
  ...
  it("calls notFound for invalid theme", () => {
    expect(() => render(<Nivel11 params={{ theme: "invalid" }} />)).toThrow("NEXT_NOT_FOUND");
    expect(notFound).toHaveBeenCalled();
  });

  it("renders play button when not started", () => {
    render(<Nivel11 params={{ theme: "alimentos" }} />);
    expect(screen.getByAltText("play")).toBeInTheDocument();
    expect(screen.queryByText(/Nível 11 - Fase/)).not.toBeInTheDocument();
  });
  ...
  it("renders layout elements", () => {
    ...
    expect(backButton).toHaveAttribute("href", "nivel11/alimentos");
  });
  ...
  it("renders game UI in early phase (syllable buttons)", () => {
    ...
    render(<Nivel11 params={{ theme: "alimentos" }} />);
    expect(screen.getByText("Nível 11 - Fase 1")).toBeInTheDocument();
  ...
  it("renders game UI in late phase (letter grid)", () => {
    ...
    render(<Nivel11 params={{ theme: "alimentos" }} />);
    expect(screen.getByText("Nível 11 - Fase 3")).toBeInTheDocument();
  ...
  it("renders phrase buttons and calls handleClickWord in phrase phase", () => {
    ...
    render(<Nivel11 params={{ theme: "alimentos" }} />);
    const phraseButton = screen.getByText("DIANA");
