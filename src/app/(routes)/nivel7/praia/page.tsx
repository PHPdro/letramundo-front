  ...
  return (
    <div className="h-screen bg-[#b6d5f0]">
      <div className="flex flex-col h-screen p-6">
        ...
        <h1 className="text-3xl font-medium text-center text-white mb-10 z-10">Nível 7</h1>
          ...
          {Array.from({ length: 8 }, (_, index) => (
            <Link href={`praia/jogo`} key={index}>
              <button
                ...
                disabled={student?.phase < index + 1 && student.level === 7}
                className="w-[70px] h-[65px] p-2 rounded-full text-center items-center flex justify-center font-mono"
                style={{
                  backgroundColor:
                    student?.phase - 1 < index + 1 && student.level === 7 ? "#f8fafc" : "#80cd3b",
                }}
              >
                {student?.phase < index + 1 && student.level === 7 ? <LockOutlined /> : index + 1}
              </button>
            </Link>
          ))}
        </div>
