...
import Nivel6 from "../page";
import { phases } from "../../../../nivel6/phases";
import {
...
describe("Nivel6 Jogo Page", () => {
  ...
  it("calls notFound for invalid theme", () => {
    expect(() => render(<Nivel6 params={{ theme: "invalid" }} />)).toThrow("NEXT_NOT_FOUND");
    expect(notFound).toHaveBeenCalled();
  });

  it("renders play button when not started", () => {
    render(<Nivel6 params={{ theme: "alimentos" }} />);
    expect(screen.getByAltText("play")).toBeInTheDocument();
    expect(screen.queryByText(/Nível 6 - Fase/)).not.toBeInTheDocument();
  });
  ...
  it("renders layout elements", () => {
    ...
    expect(backButton).toHaveAttribute("href", "nivel6/alimentos");
  });
  ...
  it("renders game UI in early phase (syllable buttons)", () => {
    ...
    render(<Nivel6 params={{ theme: "alimentos" }} />);
    expect(screen.getByText("Nível 6 - Fase 1")).toBeInTheDocument();
  ...
  it("renders game UI in late phase (letter grid)", () => {
    ...
    render(<Nivel6 params={{ theme: "alimentos" }} />);
    expect(screen.getByText("Nível 6 - Fase 3")).toBeInTheDocument();
    const letters = ["M", "O", "V", "A", "U", "L", "F", "N", "I", "E"];
  ...
  it("calls handleClick when clicking syllable button in early phase", () => {
    ctxOverrides = { start: true, phase: 1 };
    render(<Nivel6 params={{ theme: "alimentos" }} />);
    fireEvent.click(screen.getByText("VA"));
  ...
  it("renders phrase buttons and calls handleClickWord in phrase phase", () => {
    ...
    render(<Nivel6 params={{ theme: "alimentos" }} />);
    const phraseButton = screen.getByText("NOME");
