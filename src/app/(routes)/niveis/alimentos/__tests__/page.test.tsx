...
const studentLevel5 = { name: 'Maria', level: 5 }

describe('Alimentos (niveis/alimentos/page)', () => {
  beforeEach(() => {
    localStorage.clear()
  })
  ...
  it('renders logo, avatar and back button with correct color', () => {
    ...
    expect(screen.getByTestId('back-button')).toHaveAttribute('data-color', 'red')
  })
  ...
  it('disables buttons for locked levels', () => {
    ...
    const buttons = screen.getAllByRole('button')
    // levels 1-5 enabled, 6-12 disabled
  ...
  it('links each level to correct route with alimentos theme', () => {
    ...
    links.forEach((link, index) => {
      expect(link).toHaveAttribute('href', `/nivel${index + 1}/alimentos`)
    })
  })
