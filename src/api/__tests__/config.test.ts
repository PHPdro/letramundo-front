...
describe("request", () => {
  ...
  it("uses specified method", async () => {
    ...
    expect(mockedAxios).toHaveBeenCalledWith(expect.objectContaining({ method: "POST", data: { foo: 1 } }));
  });
  ...
  it("sets 7 second timeout by default", async () => {
    ...
    expect(mockedAxios).toHaveBeenCalledWith(expect.objectContaining({ timeout: 7000 }));
  });
  ...
  it("throws not found error on 404 with empty data", async () => {
    ...
    await expect(request({ endpoint: "test" })).rejects.toBe("Recurso não encontrado");
  });
  ...
  it("builds correct baseURL from endpoint", async () => {
    ...
    expect(mockedAxios).toHaveBeenCalledWith(
      expect.objectContaining({
        baseURL: "https://letramundo-back-main-axwiuq.laravel.cloud/api/login",
      }),
    );
  });
});
